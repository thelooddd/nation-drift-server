...
public OnPlayerConnect(playerid) {
    ...
    if (ServerInfo[PartNameKick] == 1) {
        for (new s = 0; s < BadPartNameCount; s++) {
            new pos;
            while ((pos = strfind(PlayerName, BadPartNames[s], true)) != -1)
                for (new i = pos, j = pos + strlen(BadPartNames[s]); i < j; i++) {
    ...
    //-----------------------------------------------------
    if (!udb_Exists(PlayerName2(playerid))) SendClientMessage(playerid, orange, "АККАУНТ: Введите /register [пароль] чтобы зарегистрироваться");
    else {
        ...
        if ((!strcmp(tmp3, tmp2, true)) && (ServerInfo[AutoLogin] == 1)) {
            ...
            if (PlayerInfo[playerid][Level] > 0) {
                format(string, sizeof(string), "*АККАУНТ: Вы автоматически залогинены. (Уровень %d)", PlayerInfo[playerid][Level]);
                SendClientMessage(playerid, green, string);
            } else SendClientMessage(playerid, green, "АККАУНТ: Вы автоматически залогинены.");
        } else SendClientMessage(playerid, green, "АККАУНТ: Ваш ник зарегистрирован,  напишите /login [пароль] чтобы войти");
    }
    return 1;
}
...
public OnPlayerDisconnect(playerid, reason) {
    ...
    if (ServerInfo[ConnectMessages] == 1) {
        switch (reason) {
            case 0:
                format(str, sizeof(str), "*** %s (%d) вышел(ла) из игры (Вылет)", PlayerName, playerid);
            case 1:
                format(str, sizeof(str), "*** %s (%d) вышел(ла) из игры (Выход)", PlayerName, playerid);
            case 2:
                format(str, sizeof(str), "*** %s (%d) вышел(ла) из игры (Кикнут либо забанен)", PlayerName, playerid);
        }
...
public OnPlayerSpawn(playerid) {
    ...
    if (PlayerInfo[playerid][Frozen] == 1) {
        ...
        return SendClientMessage(playerid, red, " Вам не поможет esc, вы наказаны и все еще заморожены");
    }

    if (PlayerInfo[playerid][Jailed] == 1) {
        SetTimerEx("JailPlayer", 3000, 0, "d", playerid);
        return SendClientMessage(playerid, red, " Вам не поможет esc, вы наказаны и все еще в тюрьме");
    }
...
public OnPlayerDeath(playerid, killerid, reason) {
    if (InDuel[playerid] == 1 && InDuel[killerid] == 1) {
        GameTextForPlayer(playerid, "LOX!!!", 3000, 3);
        InDuel[playerid] = 0;
        GameTextForPlayer(killerid, "KLACC!!!", 3000, 3);
        InDuel[killerid] = 0;
        SetPlayerPos(killerid, 0.0, 0.0, 0.0);
        SpawnPlayer(killerid);
    } else if (InDuel[playerid] == 1 && InDuel[killerid] == 0) { GameTextForPlayer(playerid, "LOX!!!", 3000, 3);
        InDuel[playerid] = 0; }
...
public OnPlayerText(playerid, text[]) {
    ...
    if (text[0] == '#' && PlayerInfo[playerid][Level] >= 1) {
        ...
        format(string, sizeof(string), " Админ чат: %s: %s", string, text[1]);
    ...
    if (ServerInfo[AntiSpam] == 1 && (PlayerInfo[playerid][Level] == 0 && !IsPlayerAdmin(playerid))) {
        ...
        if (TimeStamp() - PlayerInfo[playerid][SpamTime] > SPAM_TIMELIMIT) { // Its OK your messages were far enough apart
            ...
            new string[64];
            format(string, sizeof(string), " %s был кикнут системой (Флуд/Спам)", PlayerName2(playerid));
            SendClientMessageToAll(COLOR_RED, string);
            print(string);
            SaveToFile("KickLog", string);
            Kick(playerid);
    ...
    format(ChatSTR, 128, "[lchat]%s: %s", ChatSTR, text[0]);
...
public HighLight(playerid) {
    ...
    if (PlayerInfo[playerid][blipS] == 0) { SetPlayerColor(playerid, 0xFF0000AA);
        PlayerInfo[playerid][blipS] = 1; } else { SetPlayerColor(playerid, 0x33FF33AA);
...
dcmd_giveweapon(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        ...
        if (!strlen(tmp) || !strlen(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /giveweapon [id игрока] [id оружия или название] [патроны]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Вы дали игроку %s оружие %s(ID:%d) с %d патронами", PlayerName2(player1), WeapName, weap, ammo);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s дал вам оружие %s(ID:%d) с %d патронами", PlayerName2(playerid), WeapName, weap, ammo);
...
dcmd_sethealth(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 5) {
        ...
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /sethealth [id игрока] [кол-во здоровья]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Вы установили игроку %s здоровье %d", pName(player1), health);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам здоровье на %d", pName(playerid), health);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setarmour(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 5) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setarmour [id игрока] [кол-во брони]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Вы установили игроку %s броню %d", pName(player1), armour);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам броню на %d", pName(playerid), armour);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setcash(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 12) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setcash [id игрока] [сумма]");
        new player1 = strval(tmp), cash = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SETCASH");
            format(string, sizeof(string), " Вы установили игроку %s сумму $%d", pName(player1), cash);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам сумму $%d", pName(playerid), cash);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setscore(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 12) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setscore [id игрока] [score]");
        new player1 = strval(tmp), score = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SETSCORE");
            format(string, sizeof(string), " Вы установили игроку %s рекорд %d", pName(player1), score);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам рекорд %d", pName(playerid), score);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setskin(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setskin [id игрока] [id скина]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Вы установили игроку %s скин %d", pName(player1), skin);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %sустановил вам скин %d", pName(playerid), skin);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setwanted(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setwanted [id игрока] [уровень]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Вы установили игроку %s уровень розыскиваемости %d", pName(player1), wanted);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам уровень розыскиваемости %d", pName(playerid), wanted);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setname(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 5) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setname [id игрока] [новое имя]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Вы установили %s новое имя <<%s>>", pName(player1), tmp2);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам новое имя <<%s>>", pName(playerid), tmp2);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setcolour(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        ...
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) {
            SendClientMessage(playerid, red, " ПРАВКА: /setcolour [id игрока] [№ цвета]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            switch (Colour) {
                case 0 :  { SetPlayerColor(player1, black);
                    colour = "черный"; }
                case 1 :  { SetPlayerColor(player1, COLOR_WHITE);
                    colour = "белый"; }
                case 2 :  { SetPlayerColor(player1, red);
                    colour = "красный"; }
                case 3 :  { SetPlayerColor(player1, orange);
                    colour = "оранжевый"; }
                case 4 :  { SetPlayerColor(player1, orange);
                    colour = "желтый"; }
                case 5 :  { SetPlayerColor(player1, COLOR_GREEN1);
                    colour = "зеленый"; }
                case 6 :  { SetPlayerColor(player1, COLOR_BLUE);
                    colour = "синий"; }
                case 7 :  { SetPlayerColor(player1, COLOR_PURPLE);
                    colour = "фиолетовый"; }
                case 8 :  { SetPlayerColor(player1, COLOR_BROWN);
                    colour = "коричневый"; }
                case 9 :  { SetPlayerColor(player1, COLOR_PINK);
                    colour = "розовый"; }
            }
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s сменил вам цвет на %s ", pName(playerid), colour);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_setweather(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 5) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setweather [id игрока] [id погоды]");
        new player1 = strval(tmp), weather = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SETWEATHER");
            format(string, sizeof(string), " Вы сменили погоду %s на %d", pName(player1), weather);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s сменил погоду на %d", pName(playerid), weather);
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_settime(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 5) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /settime [id игрока] [время]");
        new player1 = strval(tmp), time = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SETTIME");
            format(string, sizeof(string), " Вы сменили игроку %s время на %d:00", pName(player1), time);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s сменил вам время на %d:00", pName(playerid), time);
                SendClientMessage(player1, COLOR_GREEN, string); }
            PlayerPlaySound(player1, 1057, 0.0, 0.0, 0.0);
...
dcmd_setworld(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 5) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setworld [id игрока] [виртуальный мир]");
        new player1 = strval(tmp), time = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SETWORLD");
            format(string, sizeof(string), " Вы установили игроку %s виртуальный мир на %d", pName(player1), time);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам виртуальный мир на %d", pName(playerid), time);
                SendClientMessage(player1, COLOR_GREEN, string); }
            PlayerPlaySound(player1, 1057, 0.0, 0.0, 0.0);
...
dcmd_setinterior(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /setinterior [id игрока] [interior]");
        new player1 = strval(tmp), time = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SETINTERIOR");
            format(string, sizeof(string), " Вы установили интерьер игроку %s на %d", pName(player1), time);
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил вам интерьер на %d", pName(playerid), time);
                SendClientMessage(player1, COLOR_GREEN, string); }
            PlayerPlaySound(player1, 1057, 0.0, 0.0, 0.0);
...
dcmd_setmytime(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setmytime [время]");
        new time = strval(params), string[128];
        CMDMessageToAdmins(playerid, "SETMYTIME");
        format(string, sizeof(string), " Вы установили себе время %d:00", time);
        SendClientMessage(playerid, COLOR_GREEN, string);
...
dcmd_force(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 4) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /force [id игрока]");
        new player1 = strval(params), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "FORCE");
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s заставил вас заного выбрать скин", pName(playerid));
                SendClientMessage(player1, COLOR_RED, string); }
            format(string, sizeof(string), " Вы заставили игрока %s заного выбрать скин", pName(player1));
...
dcmd_eject(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /eject [id игрока]");
        new player1 = strval(params), string[128], Float:x, Float:y, Float:z;
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (IsPlayerInAnyVehicle(player1)) {
                CMDMessageToAdmins(playerid, "EJECT");
                if (player1 != playerid) { format(string, sizeof(string), " Админ %s выкинул вас из транспорта", pName(playerid));
                    SendClientMessage(player1, COLOR_RED, string); }
                format(string, sizeof(string), " Вы выкинули игрока %s из транспорта", pName(player1));
                SendClientMessage(playerid, COLOR_RED, string);
...
dcmd_lockcar(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 1) {
        if (IsPlayerInAnyVehicle(playerid)) {
            ...
            new string[128];
            format(string, sizeof(string), " Админ %s закрыл двери своего транспорта", pName(playerid));
...
dcmd_unlockcar(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 1) {
        if (IsPlayerInAnyVehicle(playerid)) {
            ...
            PlayerInfo[playerid][DoorsLocked] = 0;
            new string[128];
            format(string, sizeof(string), " Админ %s открыл двери своего транспорта", pName(playerid));
            return SendClientMessageToAll(COLOR_GREEN, string);
...
dcmd_burn(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /burn [id игрока]");
        new player1 = strval(params), string[128], Float:x, Float:y, Float:z;
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "BURN");
            format(string, sizeof(string), " Вы подожгли %s", pName(player1));
            SendClientMessage(playerid, COLOR_RED, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s вас поджог", pName(playerid));
                SendClientMessage(player1, COLOR_RED, string); }
            GetPlayerPos(player1, x, y, z);
...
dcmd_spawn(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /spawn [id игрока]");
        new player1 = strval(params), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "SPAWNPLAYER");
            format(string, sizeof(string), " Вы вернули игрока %s на место спавна", pName(player1));
            SendClientMessage(playerid, COLOR_GREEN, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s вернул вас на место спавна", pName(playerid));
                SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_disarm(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /disarm [id игрока]");
        new player1 = strval(params), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "DISARM");
            PlayerPlaySound(player1, 1057, 0.0, 0.0, 0.0);
            format(string, sizeof(string), " Вы разоружили игрока %s", pName(player1));
            SendClientMessage(playerid, COLOR_RED, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s разоружил вас", pName(playerid));
                SendClientMessage(player1, COLOR_RED, string); }
...
dcmd_crash(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /crash [id игрока]");
        new player1 = strval(params), string[128], Float:X, Float:Y, Float:Z;
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
...
dcmd_ip(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /ip [id игрока]");
        new player1 = strval(params), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
...
dcmd_bankrupt(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /bankrupt [id игрока]");
        new player1 = strval(params), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "BANKRUPT");
            format(string, sizeof(string), " Вы сделали банкротом игрока %s", pName(player1));
            SendClientMessage(playerid, COLOR_RED, string);
            if (player1 != playerid) { format(string, sizeof(string), " Админ %s сделал вас банкротом", pName(playerid));
                SendClientMessage(player1, COLOR_RED, string); }
...
dcmd_sbankrupt(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 10) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /sbankrupt [id игрока]");
        new player1 = strval(params), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            CMDMessageToAdmins(playerid, "BANKRUPT");
            format(string, sizeof(string), " Вы сделали игрока %s банкротом", pName(player1));
            SendClientMessage(playerid, COLOR_RED, string);
            return ResetPlayerMoney(player1);
        } else return SendClientMessage(playerid, red, " ОШИБКА:  Нет такого игрока");
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
...
dcmd_ubound(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /ubound [id игрока]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            format(string, sizeof(string), " Админ %s удалил ваши мировые границы", PlayerName2(playerid));
            if (player1 != playerid) SendClientMessage(player1, COLOR_GREEN, string);
...
dcmd_lcommands(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] > 11) {
        SendClientMessage(playerid, green, "(11) FLIP, JAIL, REPAIR, LP, CARCOLOUR, LTUNE, SETMYTIME, TIME, GETID, LINKCAR, LNOS, LHY");
        SendClientMessage(playerid, green, "(11) ANNOUNCE");
    }
    if (PlayerInfo[playerid][Level] > 0) {
    ...
    if (PlayerInfo[playerid][Level] > 3) {
        SendClientMessage(playerid, COLOR_GREEN, "(4)DUEL, COUNTDOWN, GETHERE, TELEPLAYER, CAR, CARHEALTH, DESTROY, LTIME, LWEATHER, CAPS, FORCE");
    }
    if (PlayerInfo[playerid][Level] > 4) {
...
dcmd_getinfo(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /getinfo [id игрока]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            if (PlayerInfo[player1][Jailed] == 1) P1Jailed = "Да";
            else P1Jailed = "Нет";
            if (PlayerInfo[player1][Frozen] == 1) P1Frozen = "Да";
            else P1Frozen = "Нет";
            if (PlayerInfo[player1][LoggedIn] == 1) P1Logged = "Да";
            else P1Logged = "Нет";
            if (fexist(file)) P1Register = "Да";
            else P1Register = "Нет";
            if (dUserINT(PlayerName2(player1)).("LastOn") == 0) tmp2 = "Never";
            else tmp2 = dini_Get(file, "LastOn");
            if (strlen(dini_Get(file, "RegisteredDate")) < 3) RegDate = "n/a";
            ...
            new slot, ammo, weap, Count, WeapName[24], WeapSTR[128], p;
            WeapSTR = " Оружия: ";
                ...
                for (slot = 0; slot < 14; slot++) {
                    ...
                    if (ammo > 0 && weap > 0) {
                        ...
                        if (p >= 5) {
                            ...
                            format(WeapSTR, sizeof(WeapSTR), "Weaps: ");
                            p = 0;
    ...
    if (PlayerInfo[playerid][Level] >= 4) {
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (IsPlayerConnected(player2) && player2 != INVALID_PLAYER_ID) {
                ...
                SendClientMessage(player1, COLOR_GREEN, string);
                SendClientMessage(player2, COLOR_GREEN, string);
                return SendClientMessage(playerid, COLOR_GREEN, string);
...
dcmd_car(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 4) {
        ...
        if (!strlen(tmp)) return SendClientMessage(playerid, red, " ПРАВКА: /car [id модели или имя] [цвет 1] [цвет 2]");
...
dcmd_carhealth(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 4) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /carhealth [id игрока] [здоровье]");
        new player1 = strval(tmp), health = strval(tmp2), string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (IsPlayerInAnyVehicle(player1)) {
                CMDMessageToAdmins(playerid, "CARHEALTH");
                format(string, sizeof(string), " Вы установили игроку %s здоровье транспорта на %d", pName(player1), health);
                SendClientMessage(playerid, COLOR_GREEN, string);
                if (player1 != playerid) { format(string, sizeof(string), " Админ %s установил здоровье вашего транспорта на%d", pName(playerid), health);
                    SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_carcolour(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1) {
        ...
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        tmp3 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !strlen(tmp3) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, "ПРАВКА: /carcolour [id игрока] [цвет 1] [цвет 2]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (IsPlayerInAnyVehicle(player1)) {
                ...
                format(string, sizeof(string), " Вы изменили цвет игроку %s транспорта %s на %d,%d", pName(player1), VehicleNames[GetVehicleModel(GetPlayerVehicleID(player1)) - 400], colour1, colour2);
                SendClientMessage(playerid, COLOR_GREEN, string);
                if (player1 != playerid) { format(string, sizeof(string), " Админ %s изменил цвет вашего транспорта %s на %d,%d", pName(playerid), VehicleNames[GetVehicleModel(GetPlayerVehicleID(player1)) - 400], colour1, colour2);
                    SendClientMessage(player1, COLOR_GREEN, string); }
...
dcmd_getid(playerid, params[]) {
    if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /getid [ник]");
...
dcmd_asay(playerid, params[]) {
	#pragma unused params
    if (PlayerInfo[playerid][Level] >= 2) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /asay [текст]");
        new string[128];
        format(string, sizeof(string), "**Админ %s: %s", PlayerName2(playerid), params[0]);
        for (new i = 0; i < MAX_PLAYERS; i++) { PlayerPlaySound(i, 1130, 0.0, 0.0, 10.0); }
...
dcmd_setping(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 7 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setping [пинг] (0-чтобы выключить)");
...
dcmd_ping(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /ping [id игрока]");
        new player1 = strval(params), string[128];
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            new Sum, Average, x;
...
dcmd_highlight(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /highlight [id игрока]");
...
dcmd_setgravity(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 10) {
        ...
        format(string, sizeof(string), "Admnistrator %s has set the gravity to %f", adminname, Gravity);
        SetGravity(Gravity);
...
dcmd_serverinfo(playerid, params[]) {
	#pragma unused params
    new TotalVehicles = CreateVehicle(411, 0, 0, 0, 0, 0, 0, 1000);
    DestroyVehicle(TotalVehicles);
...
dcmd_announce(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /announce <текст>");
    ...
    if (!strlen(params)) {
        ...
        if (PlayerInfo[playerid][Level] >= 4) {
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
                ...
                GetPlayerName(player1, playername, sizeof(playername));
                GetPlayerName(playerid, adminname, sizeof(adminname));
                format(string, sizeof(string), " Админ %s дал вам летающий ранец", adminname);
                SendClientMessage(player1, COLOR_GREEN, string);
    ...
    if (PlayerInfo[playerid][Level] >= 1 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) {
            if (IsPlayerInAnyVehicle(playerid)) {
                ...
                return SendClientMessage(playerid, COLOR_GREEN, " Ваша тачка флипована. Флиповать тачку игрока /flip [id]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
            ...
            if (IsPlayerInAnyVehicle(player1)) {
                ...
                GetPlayerName(player1, playername, sizeof(playername));
                GetPlayerName(playerid, adminname, sizeof(adminname));
                format(string, sizeof(string), " Админ %s флиповал вашу тачку", adminname);
                SendClientMessage(player1, COLOR_GREEN, string);
...
dcmd_teleplayer(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 4 || IsPlayerAdmin(playerid)) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !IsNumeric(tmp) || !IsNumeric(tmp2)) return SendClientMessage(playerid, red, "ОШИБКА: /warp или /teleplayer [id 1-ого игрока] к [id 2-ого игрока]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (IsPlayerConnected(player2) && player2 != INVALID_PLAYER_ID) {
                ...
                SendClientMessage(player1, COLOR_GREEN, string);
                SendClientMessage(player2, blue, string);
...
dcmd_goto(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /goto [id игрока]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
            ...
            format(string, sizeof(string), " Вы телепортировались к %s", playername);
            return SendClientMessage(playerid, COLOR_GREEN, string);
        } else return SendClientMessage(playerid, red, " Нет такого игрока");
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас нет доступа к этой команде");
}
...
dcmd_vgoto(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /vgoto [id машины]");
        new player1, string[128];
        player1 = strval(params);
...
dcmd_vget(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /vget [id машины]");
        new player1, string[128];
        player1 = strval(params);
...
dcmd_lgoto(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2 || IsPlayerAdmin(playerid)) {
        ...
        format(string, sizeof(string), " Вы телепортированы на %f, %f, %f", x, y, z);
        return SendClientMessage(playerid, COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас нет доступа к этой команде");
}
...
dcmd_givecar(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /givecar [id игрока]");
        new player1 = strval(params), playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerInAnyVehicle(player1)) return SendClientMessage(playerid, red, " ОШИБКА: У игрока есть машина");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
            ...
            GetPlayerName(player1, playername, sizeof(playername));
            GetPlayerName(playerid, adminname, sizeof(adminname));
            format(string, sizeof(string), " Админ %s дал вам машину", adminname);
            SendClientMessage(player1, COLOR_GREEN, string);
            format(string, sizeof(string), " Вы дали игроку %s машину", playername);
            return SendClientMessage(playerid, COLOR_GREEN, string);
        } else return SendClientMessage(playerid, red, " Нет такого игрока");
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас нет доступа к этой команде");
}
...
dcmd_gethere(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 4) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /gethere [id игрока]");
        new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
        player1 = strval(params);
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
            ...
            GetPlayerName(player1, playername, sizeof(playername));
            GetPlayerName(playerid, adminname, sizeof(adminname));
            format(string, sizeof(string), " Админ %s телепортировал вас к себе", adminname);
            SendClientMessage(player1, COLOR_GREEN, string);
            format(string, sizeof(string), " Вы телепортировали %s к себе", playername);
            return SendClientMessage(playerid, COLOR_GREEN, string);
        } else return SendClientMessage(playerid, red, " Нет такого игрока");
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас нет доступа к этой команде");
}
...
dcmd_get(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /get [id игрока]");
        new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
        player1 = strval(params);
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid) {
            ...
            GetPlayerName(player1, playername, sizeof(playername));
            GetPlayerName(playerid, adminname, sizeof(adminname));
            format(string, sizeof(string), " Админ %s доставил вас к себе", adminname);
            SendClientMessage(player1, COLOR_GREEN, string);
            format(string, sizeof(string), " Вы доставили игрока %s к себе", playername);
            return SendClientMessage(playerid, COLOR_GREEN, string);
        } else return SendClientMessage(playerid, red, " Нет такого игрока");
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас нет доступа к этой команде");
}
...
dcmd_fu(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /fu [id игрока]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            ...
            SetPlayerHealth(player1, 1.0);
            SetPlayerArmour(player1, 0.0);
            ResetPlayerWeapons(player1);
            ResetPlayerMoney(player1);
            GivePlayerWeapon(player1, 12, 1);
            SetPlayerSkin(player1, 137);
            SetPlayerScore(player1, 0);
            SetPlayerColor(player1, COLOR_PINK);
            SetPlayerWeather(player1, 19);
            SetPlayerWantedLevel(player1, 6);
            format(NewName, sizeof(NewName), "[Hy6]%s", pName(player1));
            SetPlayerName(player1, NewName);
            if (IsPlayerInAnyVehicle(player1)) EraseVehicle(GetPlayerVehicleID(player1));
            if (player1 != playerid) { format(string, sizeof(string), " ~w~%s: ~r~TS HY6", pName(playerid));
                GameTextForPlayer(player1, string, 2500, 3); }
            format(string, sizeof(string), " Вы сделали нубом %s", pName(player1));
...
dcmd_warn(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /warn [id игрока] [причина]");
...
dcmd_kick(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 3) {
            new tmp[256], tmp2[256], Index;
            tmp = strtok(params, Index), tmp2 = strtok(params, Index);
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /kick [id] [причина]");
            for (new i = 0; i < MAX_PLAYERS; i++) { PlayerPlaySound(i, 1058, 0.0, 0.0, 10.0); }
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(tmp);
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
                ...
                if (!strlen(tmp2)) {
                    format(string, sizeof(string), " Игрока %s кикнул админ %s [нет причины] ", playername, adminname);
                    SendClientMessageToAll(COLOR_RED, string);
                    SaveToFile("KickLog", string);
                    print(string);
                    return Kick(player1);
                } else {
                    format(string, sizeof(string), " %s кикнул админ %s [причина: %s] ", playername, adminname, params[2]);
                    SendClientMessageToAll(COLOR_RED, string);
                    SaveToFile("KickLog", string);
                    print(string);
                    return Kick(player1);
...
dcmd_ban(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 6) {
            new tmp[256], tmp2[256], Index;
            tmp = strtok(params, Index), tmp2 = strtok(params, Index);
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /ban [id] [причина]");
            for (new i = 0; i < MAX_PLAYERS; i++) { PlayerPlaySound(i, 1136, 0.0, 0.0, 10.0); }
...
dcmd_rban(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 9) {
            ...
            tmp = strtok(params, Index), tmp2 = strtok(params, Index);
            if (!strlen(params)) return SendClientMessage(playerid, red, " ОШИБКА: /rban [id игрока] [причина]");
            if (!strlen(tmp2)) return SendClientMessage(playerid, red, " ОШИБКА: Вы должны ввести причину");
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(tmp);
...
dcmd_slap(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 2) {
            new tmp[256], tmp2[256], Index;
            tmp = strtok(params, Index), tmp2 = strtok(params, Index);
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /slap [id игрока] [причина]");
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(tmp);
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
                ...
                if (strlen(tmp2)) {
                    format(string, sizeof(string), " Вас пнул администратор %s по причине %s ", adminname, params[2]);
                    SendClientMessage(player1, COLOR_RED, string);
                    format(string, sizeof(string), " Вы пнули игрока %s по причине %s ", playername, params[2]);
                    return SendClientMessage(playerid, COLOR_RED, string);
                } else {
                    format(string, sizeof(string), " Вас пнул администратор %s ", adminname);
                    SendClientMessage(player1, COLOR_RED, string);
                    format(string, sizeof(string), " Вы пнули игрока %s", playername);
                    return SendClientMessage(playerid, COLOR_RED, string);
                }
            } else return SendClientMessage(playerid, red, " Нет такого игрока");
        } else return SendClientMessage(playerid, red, " ОШИБКА: У вас нет доступа к этой команде");
...
dcmd_explode(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 3) {
            new tmp[256], tmp2[256], Index;
            tmp = strtok(params, Index), tmp2 = strtok(params, Index);
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /explode [id игрока] [причина]");
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(tmp);
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
                ...
                if (strlen(tmp2)) {
                    format(string, sizeof(string), " Вы были взорваны админом %s [причина: %s]", adminname, params[2]);
                    SendClientMessage(player1, COLOR_RED, string);
                    format(string, sizeof(string), " Вы взорвали игрока %s [причина: %s]", playername, params[2]);
                    return SendClientMessage(playerid, COLOR_RED, string);
                } else {
                    format(string, sizeof(string), " Вы были взорваны админом %s", adminname);
                    SendClientMessage(player1, COLOR_RED, string);
                    format(string, sizeof(string), " Вы взорвали игрока %s", playername);
                    return SendClientMessage(playerid, COLOR_RED, string);
                }
        ...
        if (PlayerInfo[playerid][Level] >= 3) {
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
                if (PlayerInfo[player1][Jailed] == 1) {
                    GetPlayerName(player1, playername, sizeof(playername));
                    GetPlayerName(playerid, adminname, sizeof(adminname));
                    format(string, sizeof(string), " Админ %s выпустил тебя", adminname);
                    SendClientMessage(player1, COLOR_GREEN, string);
    ...
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 2) {
            ...
            for (i = 0; i < MAX_PLAYERS; i++)
                if (IsPlayerConnected(i) && PlayerInfo[i][Jailed]) Count++;
            if (Count == 0) return SendClientMessage(playerid, red, " Нет посаженых игроков");

            for (i = 0; i < MAX_PLAYERS; i++)
                if (IsPlayerConnected(i) && PlayerInfo[i][Jailed]) {
                    GetPlayerName(i, adminname, sizeof(adminname));
                    if (!First2) { format(string, sizeof(string), " Посаженые игроки: (%d)%s", i, adminname);
                        First2 = true; } else format(string, sizeof(string), "%s, (%d)%s ", string, i, adminname);
                }
...
dcmd_freeze(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 3) {
            new tmp[256], tmp2[256], tmp3[256], Index;
            tmp = strtok(params, Index), tmp2 = strtok(params, Index), tmp3 = strtok(params, Index);
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /freeze [id игрока] [минуты] [причина]");
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(tmp);
...
dcmd_unfreeze(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 3 || IsPlayerAdmin(playerid)) {
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /unfreeze [id игрока]");
            new player1, string[128];
            player1 = strval(params);
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
                if (PlayerInfo[player1][Frozen] == 1) {
                    ...
                    UnFreezeMe(player1);
                    format(string, sizeof(string), " Админ %s разморозил вас", PlayerName2(playerid));
                    SendClientMessage(player1, COLOR_GREEN, string);
...
dcmd_frozen(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 2) {
            new bool:First2 = false, Count, adminname[MAX_PLAYER_NAME], string[128], i;
            for (i = 0; i < MAX_PLAYERS; i++)
                if (IsPlayerConnected(i) && PlayerInfo[i][Frozen]) Count++;
            if (Count == 0) return SendClientMessage(playerid, red, " Нет замороженых игроков");

            for (i = 0; i < MAX_PLAYERS; i++)
                if (IsPlayerConnected(i) && PlayerInfo[i][Frozen]) {
                    GetPlayerName(i, adminname, sizeof(adminname));
                    if (!First2) { format(string, sizeof(string), " Замороженые игроки: (%d)%s", i, adminname);
                        First2 = true; } else format(string, sizeof(string), "%s, (%d)%s ", string, i, adminname);
                }
            return SendClientMessage(playerid, COLOR_WHITE, string);
        } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
        ...
        if (PlayerInfo[playerid][Level] >= 2) {
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
                if (PlayerInfo[player1][Muted] == 0) {
                    ...
                    if (strlen(tmp2)) {
                        format(string, sizeof(string), " Вы были заткнуты админом %s [причина: %s]", adminname, params[2]);
                        SendClientMessage(player1, COLOR_RED, string);
                        format(string, sizeof(string), " Вы заткнули игрока %s [причина: %s]", playername, params[2]);
                        return SendClientMessage(playerid, COLOR_RED, string);
                    } else {
                        format(string, sizeof(string), " Вы были заткнуты админом %s", adminname);
                        SendClientMessage(player1, COLOR_RED, string);
                        format(string, sizeof(string), " Вы заткнули игрока %s", playername);
                        return SendClientMessage(playerid, COLOR_RED, string);
                    }
...
dcmd_unmute(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 2) {
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /unmute [id игрока]");
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(params);
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
                if (PlayerInfo[player1][Muted] == 1) {
                    ...
                    PlayerInfo[player1][MuteWarnings] = 0;
                    format(string, sizeof(string), " Вам дал свободу голоса админ %s", adminname);
                    SendClientMessage(player1, COLOR_GREEN, string);
                    format(string, sizeof(string), " Вы разоткнули игрока %s", playername);
                    return SendClientMessage(playerid, COLOR_GREEN, string);
...
dcmd_muted(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 2) {
            new bool:First2 = false, Count, adminname[MAX_PLAYER_NAME], string[128], i;
            for (i = 0; i < MAX_PLAYERS; i++)
                if (IsPlayerConnected(i) && PlayerInfo[i][Muted]) Count++;
            if (Count == 0) return SendClientMessage(playerid, red, " Нет заткнутых игроков");

            for (i = 0; i < MAX_PLAYERS; i++)
                if (IsPlayerConnected(i) && PlayerInfo[i][Muted]) {
                    GetPlayerName(i, adminname, sizeof(adminname));
                    if (!First2) { format(string, sizeof(string), " Заткнутые игроки: (%d)%s", i, adminname);
                        First2 = true; } else format(string, sizeof(string), "%s, (%d)%s ", string, i, adminname);
                }
            return SendClientMessage(playerid, COLOR_WHITE, string);
        } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
...
dcmd_akill(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 3 || IsPlayerAdmin(playerid)) {
            if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /akill [id игрока]");
            new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
            player1 = strval(params);
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
                ...
                GetPlayerName(player1, playername, sizeof(playername));
                GetPlayerName(playerid, adminname, sizeof(adminname));
                format(string, sizeof(string), " Админ %s убил вас", adminname);
                SendClientMessage(player1, COLOR_RED, string);
                format(string, sizeof(string), " Вы убили игрока %s", playername);
                SendClientMessage(playerid, COLOR_RED, string);
                return SetPlayerHealth(player1, 0.0);
            } else return SendClientMessage(playerid, red, " Нет такого игрока");
        } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
...
dcmd_weaps(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /weaps [id игрока]");
        new player1, string[128], string2[64], WeapName[24], slot, weap, ammo, Count, x;
        player1 = strval(params);

        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            format(string2, sizeof(string2), "[>> %s Оружия (id:%d) <<]", PlayerName2(player1), player1);
...
dcmd_aka(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /aka [id игрока]");
...
dcmd_screen(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /screen [id игрока] [текст]");
        new player1, playername[MAX_PLAYER_NAME], adminname[MAX_PLAYER_NAME], string[128];
        player1 = strval(params);
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID && player1 != playerid && (PlayerInfo[player1][Level] != ServerInfo[MaxAdminLevel])) {
            ...
            format(string, sizeof(string), " Админ %s отправил вам сообщение на экран", adminname);
            SendClientMessage(player1, COLOR_GREEN, string);
            format(string, sizeof(string), " Вы отправили игроку %s сообщение на экран (%s)", playername, params[2]);
            SendClientMessage(playerid, COLOR_GREEN, string);
...
dcmd_admins(playerid, params[]) {
    ...
    if (Count[0] == 1) {
        for (i = 0; i < MAX_PLAYERS; i++)
            if (IsPlayerConnected(i) && PlayerInfo[i][Level] > 0) {
                format(string, sizeof(string), " Админ: (ID:%d)%s [%d]", i, PlayerName2(i), PlayerInfo[i][Level]);
                SendClientMessage(playerid, blue, string);
            }
    }
    ...
    if (Count[0] > 1) {
        new x;
        format(string, sizeof(string), " Админы: ");
        for (i = 0; i < MAX_PLAYERS; i++)
            if (IsPlayerConnected(i) && PlayerInfo[i][Level] > 0) {
    ...
    if ((Count[1] == 1) && (PlayerInfo[playerid][Level] > 0)) {
        for (i = 0; i < MAX_PLAYERS; i++)
            if (IsPlayerConnected(i) && IsPlayerAdmin(i)) {
                format(string, sizeof(string), " RCON Админ: (ID:%d)%s", i, PlayerName2(i));
                SendClientMessage(playerid, COLOR_WHITE, string);
            }
    }
    if (Count[1] > 1) {
        new x;
        format(string, sizeof(string), " RCON Админы: ");
        for (i = 0; i < MAX_PLAYERS; i++)
            if (IsPlayerConnected(i) && IsPlayerAdmin(i)) {
...
dcmd_adminarea(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 1) {
        ...
        new string[100];
        format(string, sizeof(string), "%s телепортировался в убежище для админов", PlayerName2(playerid));
        ...
        if (PlayerInfo[playerid][Level] >= 12 || IsPlayerAdmin(playerid)) {
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
                if (PlayerInfo[player1][LoggedIn] == 1) {
                    ...
                    format(string, sizeof(string), " Вы сделали игрока %s %d уровня %d/%d/%d в %d:%d:%d", playername, level, day, month, year, hour, minute, second);
                    SendClientMessage(playerid, COLOR_GREEN, string);
...
dcmd_settemplevel(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (PlayerInfo[playerid][Level] >= 12 || IsPlayerAdmin(playerid)) {
            new tmp[256], tmp2[256], Index;
            tmp = strtok(params, Index), tmp2 = strtok(params, Index);
            if (!strlen(tmp) || !strlen(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /settemplevel [id игрока] [уровень]");
            ...
            if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
                if (PlayerInfo[player1][LoggedIn] == 1) {
                    ...
                    format(string, sizeof(string), " Вы сделали игрока %s временно %d уровнем %d/%d/%d в %d:%d:%d", pName(player1), level, day, month, year, hour, minute, second);
                    SendClientMessage(playerid, blue, string);
...
dcmd_report(playerid, params[]) {
    ...
    if (IsPlayerConnected(reported) && reported != INVALID_PLAYER_ID) {
        ...
        if (strlen(params) > 7) {
            new reportedname[MAX_PLAYER_NAME], reporter[MAX_PLAYER_NAME], str[128], hour, minute, second;
            gettime(hour, minute, second);
...
dcmd_reports(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 1) {
        ...
        for (new i = 1; i < MAX_REPORTS; i++) {
            if (strcmp(Reports[i], "<none>", true) != 0) { ReportCount++;
        ...
        for (i = 0; i < MAX_PLAYERS; i++) {
            if (IsPlayerConnected(i)) {
                for (slot = 0; slot < 14; slot++) {
                    ...
                    if (ammo != 0 && weap == 38) {
                        Count++;
                        if (!First2) { format(string, sizeof(string), " Миниган: (%d)%s(патроны:%d)", i, PlayerName2(i), ammo);
                            First2 = true; } else format(string, sizeof(string), "%s, (%d)%s(патроны:%d) ", string, i, PlayerName2(i), ammo);
                    }
                }
            }
        }
        if (Count == 0) return SendClientMessage(playerid, COLOR_WHITE, " Нет игроков с миниганом");
...
dcmd_lockserver(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 12) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /lockserver [пароль]");
...
dcmd_password(playerid, params[]) {
    if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /password [пароль]");
    if (ServerInfo[Locked] == 1) {
...
dcmd_forbidname(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 7) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /forbidname [ник]");
...
dcmd_forbidword(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 7) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /forbidword [слово]");
        new File:BLfile, string[128];
...
dcmd_lspec(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        if (!strlen(params) || !IsNumeric(params)) return SendClientMessage(playerid, red, " ПРАВКА: /lspec [id игрока]");
...
dcmd_lspecvehicle(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 2) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /lspecvehicle [id транспорта]");
...
dcmd_caps(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 4) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || IsNumeric(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /caps [id игрока] [on/off]");
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (strcmp(tmp2, "on", true) == 0) {
                ...
                if (player1 != playerid) { format(string, sizeof(string), " Админ %s дал вам возможность писать большими буквами", pName(playerid));
                    SendClientMessage(player1, COLOR_GREEN, string); }
                format(string, sizeof(string), " Вы дали возможность игроку %s писать большими буквами", pName(player1));
                return SendClientMessage(playerid, COLOR_GREEN, string);
            } else if (strcmp(tmp2, "off", true) == 0) {
                CMDMessageToAdmins(playerid, "CAPS");
                PlayerInfo[player1][Caps] = 1;
                if (player1 != playerid) { format(string, sizeof(string), " Админ %s отобрал у вас возможность писать большими буквами", pName(playerid));
                    SendClientMessage(player1, COLOR_RED, string); }
                format(string, sizeof(string), " Вы отобрли возможность игроку %s писать большими буквами", pName(player1));
                return SendClientMessage(playerid, COLOR_RED, string);
...
dcmd_invis(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 7) {
        if (IsPlayerInAnyVehicle(playerid)) {
            ...
            LVehicleID = CreateVehicle(411, X, Y, Z, Angle, 1, -1, -1);
...
dcmd_uninvis(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 7) {
        if (PlayerInfo[playerid][Invis] == 1) {
            ...
            new InvisCount;
            for (new i = 0; i < MAX_PLAYERS; i++)
                if (PlayerInfo[i][Invis] == 1) InvisCount++;
            if (InvisCount == 0)
                if (InvisTimer) KillTimer(InvisTimer);
...
dcmd_pickup(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 10 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /pickup [id пикапа]");
...
dcmd_object(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 10 || IsPlayerAdmin(playerid)) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /object [id обьекта]");
...
dcmd_move(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /move [up/down/+x/-x/+y/-y/off]");
        new Float:X, Float:Y, Float:Z;
        ...
        } else return SendClientMessage(playerid, red, " ПРАВКА: /move [up/down/+x/-x/+y/-y/off]");
        return 1;
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
    ...
    if (PlayerInfo[playerid][Level] >= 3) {
        ...
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
            if (strcmp(tmp2, "up", true) == 0) {
                ...
                GetPlayerPos(player1, X, Y, Z);
                SetPlayerPos(player1, X + 5, Y, Z);
            } else if (strcmp(tmp2, "-x", true) == 0) {
                GetPlayerPos(player1, X, Y, Z);
                SetPlayerPos(player1, X - 5, Y, Z);
            } else if (strcmp(tmp2, "+y", true) == 0) {
                GetPlayerPos(player1, X, Y, Z);
                SetPlayerPos(player1, X, Y + 5, Z);
            } else if (strcmp(tmp2, "-y", true) == 0) {
                GetPlayerPos(player1, X, Y, Z);
                SetPlayerPos(player1, X, Y - 5, Z);
...
dcmd_fakedeath(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        new tmp[256], tmp2[256], tmp3[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index), tmp3 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2) || !strlen(tmp3)) return SendClientMessage(playerid, red, " ПРАВКА: /fakedeath [убийца] [смертник] [id оружия]");
...
dcmd_fakechat(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /fakechat [id игрока] [текст]");
...
dcmd_fakecmd(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 9) {
        new tmp[256], tmp2[256], Index;
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp) || !strlen(tmp2)) return SendClientMessage(playerid, red, " ПРАВКА: /fakecmd [id игрока] [команда]");
        new player1 = strval(tmp);
        if (PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете использовать команду на этом уровне");
        if (IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
...
dcmd_spawnall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s вернул на спавн всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_muteall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s заткнул всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_unmuteall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s разоткнул всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_getall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s телепортировал всех игроков к себе", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_healall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 5) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s вылечил всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
...
dcmd_armourall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 5) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s подарил всем по полной броне", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 5 уровень");
}
...
dcmd_killall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s убил всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_freezeall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s зморозил всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_unfreezeall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s разморозил всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_kickall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s кикнул всех игроков", pName(playerid));
        SaveToFile("KickLog", string);
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_slapall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 9) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s пнул всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 9 уровень");
}
...
dcmd_explodeall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 9) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s взорвал всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 9 уровень");
}
...
dcmd_disarmall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s разоружил всех игроков", pName(playerid));
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_ejectall(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 8) {
        ...
        new string[128];
        format(string, sizeof(string), " Админ %s выкинул всех из транспорта", pName(playerid));
        return SendClientMessageToAll(COLOR_RED, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 8 уровень");
}
...
dcmd_setallskin(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 3) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setallskin [id скина]");
        new
        var = strval(params), string[128];
        ...
        format(string, sizeof(string), " Админ %s установил всем скин %d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_setallwanted(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setallwanted [уровень розыска]");
        new
        var = strval(params), string[128];
        ...
        for (new i = 0; i < MAX_PLAYERS; i++) {
            if (IsPlayerConnected(i) && i != playerid && i != ServerInfo[MaxAdminLevel]) {
                PlayerPlaySound(i, 1057, 0.0, 0.0, 0.0);
                SetPlayerWantedLevel(i,
                    var);
            }
        }
        format(string, sizeof(string), " Админ %s установил всем уровень розыскиваемости %d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_setallweather(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setallweather [id погоды]");
        new
        var = strval(params), string[128];
        ...
        format(string, sizeof(string), "Админ %s установил всем погоду %d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_setalltime(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setalltime [время]");
        new
        var = strval(params), string[128];
        ...
        format(string, sizeof(string), " Админ %s установил всем время %d:00", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_setallworld(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setallworld [виртуальный мир]");
        new
        var = strval(params), string[128];
        ...
        for (new i = 0; i < MAX_PLAYERS; i++) {
            if (IsPlayerConnected(i) && i != playerid && i != ServerInfo[MaxAdminLevel]) {
                PlayerPlaySound(i, 1057, 0.0, 0.0, 0.0);
                SetPlayerVirtualWorld(i,
                    var);
            }
        }
        format(string, sizeof(string), " Админ %s установил всем виртуальный мир %d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_setallscore(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /setallscore [рекорд]");
        new
        var = strval(params), string[128];
        ...
        format(string, sizeof(string), " Админ %s установил всем рекорд %d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_setallcash(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        ...
        format(string, sizeof(string), " Админ %s установил всем сумму на руках $%d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_giveallcash(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /giveallcash [сумма]");
        new
        var = strval(params), string[128];
        ...
        format(string, sizeof(string), " Админ %s подарил всем по $%d", pName(playerid),
            var);
        return SendClientMessageToAll(COLOR_GREEN, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
}
...
dcmd_giveallweapon(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 6) {
        ...
        tmp = strtok(params, Index), tmp2 = strtok(params, Index);
        if (!strlen(tmp)) return SendClientMessage(playerid, red, " ПРАВКА: /giveallweapon [id оружия или название оружия] [патроны]");
        if (!strlen(tmp2) || !IsNumeric(tmp2) || strval(tmp2) <= 0 || strval(tmp2) > 99999) ammo = 500;
        if (!IsNumeric(tmp)) weap = GetWeaponIDFromName(tmp);
...
dcmd_ltele(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 3) {
        if (IsPlayerInAnyVehicle(playerid)) {
            TogglePlayerControllable(playerid, false);
            return ShowMenuForPlayer(LTele, playerid);
...
dcmd_lweather(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 4) {
        if (IsPlayerInAnyVehicle(playerid)) {
            TogglePlayerControllable(playerid, false);
            return ShowMenuForPlayer(LWeather, playerid);
...
dcmd_ltime(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 4) {
        if (IsPlayerInAnyVehicle(playerid)) {
            TogglePlayerControllable(playerid, false);
            return ShowMenuForPlayer(LTime, playerid);
...
dcmd_ltmenu(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 3) {
        if (IsPlayerInAnyVehicle(playerid)) {
            new LVehicleID = GetPlayerVehicleID(playerid), LModel = GetVehicleModel(LVehicleID);
            switch (LModel) {
                case 448, 461, 462, 463, 468, 471, 509, 510, 521, 522, 523, 581, 586, 449 : return SendClientMessage(playerid, red, " ОШИБКА: Вы не можете тюнить этот транспорт!"); }
            TogglePlayerControllable(playerid, false);
            return ShowMenuForPlayer(LTuneMenu, playerid);
...
dcmd_lweapons(playerid, params[]) {
    ...
    if (PlayerInfo[playerid][Level] >= 3) {
        if (IsPlayerInAnyVehicle(playerid)) {
            TogglePlayerControllable(playerid, false);
            return ShowMenuForPlayer(XWeapons, playerid);
...
dcmd_saveskin(playerid, params[]) {
    if (PlayerInfo[playerid][Level] >= 1 && PlayerInfo[playerid][LoggedIn] == 1) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /saveskin [id скина]");
...
dcmd_alogin(playerid, params[]) {
    ...
    if (udb_CheckLogin(PlayerName2(playerid), params)) {
        ...
        if (PlayerInfo[playerid][FailLogin] == MAX_FAIL_LOGINS) {
            new string[128];
            format(string, sizeof(string), " %s кикнут системой (Причина: Несколько раз ввел не верный пароль)", PlayerName2(playerid));
            SendClientMessageToAll(COLOR_RED, string);
            print(string);
            Kick(playerid);
        }
...
dcmd_register(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) return SendClientMessage(playerid, red, " АККАУНТ: Вы зарегистрированы и залогинены.");
    if (udb_Exists(PlayerName2(playerid))) return SendClientMessage(playerid, red, " АККАУНТ: Этот ник зарегистрирован, если он ваш то залогинийтесь командой /login [password].");
    if (strlen(params) == 0) return SendClientMessage(playerid, red, " АККАУНТ: ПРАВКА: /register [пароль]");
    if (strlen(params) < 4 || strlen(params) > 20) return SendClientMessage(playerid, red, "Cервер: Пароль слишком короткий или длиный!");
    if (udb_Create(PlayerName2(playerid), params)) {
        new file[256], name[MAX_PLAYER_NAME], tmp3[100];
        new strdate[20], year, month, day;
        getdate(year, month, day);
        ...
        PlayerInfo[playerid][LoggedIn] = 1;
        PlayerInfo[playerid][Registered] = 1;
        SendClientMessage(playerid, green, " АККАУНТ: Вы успешно зарегистрировались, и автоматически залогинились");
        PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
        return 1;
    }
    return 1;
...
dcmd_login(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) return SendClientMessage(playerid, red, " АККАУНТ: Вы успешно вошли.");
    if (!udb_Exists(PlayerName2(playerid))) return SendClientMessage(playerid, red, " АККАУНТ: Учетной записи не существует, пожалуйста введите /register [пароль].");
    if (strlen(params) == 0) return SendClientMessage(playerid, red, " АККАУНТ: ПРАВКА: /login [пароль]");
    if (udb_CheckLogin(PlayerName2(playerid), params)) {
        new file[256], tmp3[100], string[128];
        format(file, sizeof(file), "/ladmin/users/%s.sav", udb_encode(PlayerName2(playerid)));
        GetPlayerIp(playerid, tmp3, 100);
        ...
        PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
        if (PlayerInfo[playerid][Level] > 0) {
            format(string, sizeof(string), " АКАУНТ: Вы успешно залогинились. (Уровень %d)", PlayerInfo[playerid][Level]);
            return SendClientMessage(playerid, green, string);
        } else return SendClientMessage(playerid, green, " АКАУНТ: Вы успешно залогинились");
    } else {
        PlayerInfo[playerid][FailLogin]++;
        ...
        return SendClientMessage(playerid, red, " АККАУНТ: Вы не залогинились! Не верный пароль");
    }
}

dcmd_changepass(playerid, params[]) {
    if (PlayerInfo[playerid][LoggedIn] == 1) {
        if (!strlen(params)) return SendClientMessage(playerid, red, " ПРАВКА: /changepass [новый пароль]");
        if (strlen(params) < 4) return SendClientMessage(playerid, red, " АККАУНТ: Не верный пароль");
        new string[128];
        dUserSetINT(PlayerName2(playerid)).("password_hash", udb_hash(params));
        dUserSet(PlayerName2(playerid)).("Password", params);
        PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
        format(string, sizeof(string), " АККАУНТ: Вы успешно сменили пароль на [ %s ]", params);
        return SendClientMessage(playerid, yellow, string);
    } else return SendClientMessage(playerid, red, " ОШИБКА: Вы должны зарегистрироваться, чтобы использовать эту команду");
}
...
LoginPlayer(playerid) {
    if (ServerInfo[GiveMoney] == 1) { ResetPlayerMoney(playerid);
...
public OnPlayerCommandText(playerid, cmdtext[]) {
    ...
    SendClientMessage(playerid, red, " Вы не можете пользоваться командами в тюрьме");
    ...
    if (strcmp(cmd, "/lp", true) == 0) {
        if (PlayerInfo[playerid][Level] >= 1) {
            if (GetPlayerState(playerid) == 2) {
                ...
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Ваша машина была изьята на прокачку");
                CMDMessageToAdmins(playerid, "LP");
            } else {
    ...
    if (strcmp(cmd, "/spam", true) == 0) {
        if (PlayerInfo[playerid][Level] >= 10) {
            ...
            if (!strlen(tmp)) {
                SendClientMessage(playerid, red, " ПРАВКА: /spam [№ цвета] [текст]");
            ...
            if (Colour == 0)
                for (new i; i < 50; i++) SendClientMessageToAll(black, string);
            else if (Colour == 1)
                for (new i; i < 50; i++) SendClientMessageToAll(COLOR_WHITE, string);
            else if (Colour == 2)
                for (new i; i < 50; i++) SendClientMessageToAll(red, string);
            else if (Colour == 3)
                for (new i; i < 50; i++) SendClientMessageToAll(orange, string);
            else if (Colour == 4)
                for (new i; i < 50; i++) SendClientMessageToAll(yellow, string);
            else if (Colour == 5)
                for (new i; i < 50; i++) SendClientMessageToAll(COLOR_GREEN1, string);
            else if (Colour == 6)
                for (new i; i < 50; i++) SendClientMessageToAll(COLOR_BLUE, string);
            else if (Colour == 7)
                for (new i; i < 50; i++) SendClientMessageToAll(COLOR_PURPLE, string);
            else if (Colour == 8)
                for (new i; i < 50; i++) SendClientMessageToAll(COLOR_BROWN, string);
            else if (Colour == 9)
                for (new i; i < 50; i++) SendClientMessageToAll(COLOR_PINK, string);
            return 1;
        } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 10 уровень");
    }
    ...
    if (strcmp(cmd, "/write", true) == 0) {
        if (PlayerInfo[playerid][Level] >= 2) {
            tmp = strtok(cmdtext, idx);
            if (!strlen(tmp)) {
                SendClientMessage(playerid, red, " ПРАВКА: /write [№ цвета] [текст]");
            ...
            Colour = strval(tmp);
            if (Colour > 9) {
                SendClientMessage(playerid, red, " ПРАВКА: /write [Colour] [Text]");
                return SendClientMessage(playerid, red, " Цвета: 0=черный 1=белый 2=красный 3=оранжевый 4=желтый 5=зеленый 6=синий 7=фиолетовый 8=коричневый 9=розовый");
            }
            tmp = strtok(cmdtext, idx);
            ...
            if (Colour == 0) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(black, string); return 1; } else if (Colour == 1) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(COLOR_WHITE, string); return 1; } else if (Colour == 2) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(red, string); return 1; } else if (Colour == 3) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(orange, string); return 1; } else if (Colour == 4) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(yellow, string); return 1; } else if (Colour == 5) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(COLOR_GREEN1, string); return 1; } else if (Colour == 6) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(COLOR_BLUE, string); return 1; } else if (Colour == 7) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(COLOR_PURPLE, string); return 1; } else if (Colour == 8) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(COLOR_BROWN, string); return 1; } else if (Colour == 9) { format(string, sizeof(string), " %s", cmdtext[9]);
                SendClientMessageToAll(COLOR_PINK, string); return 1; }
            return 1;
        } else return SendClientMessage(playerid, red, " ОШИБКА: У вас должен быть 2 уровень");
    }
...
public OnPlayerSelectedMenuRow(playerid, row) {
    ...
    if (Current == LMainMenu) {
        switch (row) {
            case 0: {
                if (PlayerInfo[playerid][Level] >= 4) {
                    ShowMenuForPlayer(AdminEnable, playerid);
                    TogglePlayerControllable(playerid, true);
                } else {
            ...
            case 1: {
                if (PlayerInfo[playerid][Level] >= 4) {
                    ShowMenuForPlayer(AdminDisable, playerid);
                    TogglePlayerControllable(playerid, true);
                } else {
                    SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
                    TogglePlayerControllable(playerid, true);
            ...
            case 4:
                ShowMenuForPlayer(LVehicles, playerid);
            case 5:
                ShowMenuForPlayer(LCars, playerid);
            case 6: {
                if (PlayerInfo[playerid][Level] >= 2) {
                    if (IsPlayerInAnyVehicle(playerid)) {
                        new LVehicleID = GetPlayerVehicleID(playerid), LModel = GetVehicleModel(LVehicleID);
                        switch (LModel) {
                            case 448, 461, 462, 463, 468, 471, 509, 510, 521, 522, 523, 581, 586, 449: { SendClientMessage(playerid, red, " ОШИБКА: Вы не можете тюнить этот транспорт");
                                TogglePlayerControllable(playerid, true); return 1; }
                        }
                        TogglePlayerControllable(playerid, false);
                        ShowMenuForPlayer(LTuneMenu, playerid);
                    } else { SendClientMessage(playerid, red, " ОШИБКА: Вы не можете тюнить этот транспорт");
                        TogglePlayerControllable(playerid, true); }
                } else { SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
                    TogglePlayerControllable(playerid, true); }
            }
            case 7 :  { if (PlayerInfo[playerid][Level] >= 3) ShowMenuForPlayer(XWeapons, playerid);
                else SendClientMessage(playerid, red, " ОШИБКА: У вас не достаточно высокий уровень чтобы использовать эту команду");
                TogglePlayerControllable(playerid, true); }
            case 8:
                ShowMenuForPlayer(LTele, playerid);
            case 9: {
                new Menu:Currentxmenu = GetPlayerMenu(playerid);
                HideMenuForPlayer(Currentxmenu, playerid);
                TogglePlayerControllable(playerid, true);
            }
        }
        return 1;
    ...
    if (Current == AdminEnable) {
        new adminname[MAX_PLAYER_NAME], file[256];
        GetPlayerName(playerid, adminname, sizeof(adminname));
        format(file, sizeof(file), "ladmin/config/Config.ini");
        switch (row) {
            case 0 :  { ServerInfo[AntiSwear] = 1;
                dini_IntSet(file, "AntiSwear", 1);
                format(string, sizeof(string), " Админ %s включил анти-мат", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 1 :  { ServerInfo[NameKick] = 1;
                dini_IntSet(file, "NameKick", 1);
                format(string, sizeof(string), " Админ %s включил кик по нику", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 2: { ServerInfo[AntiSpam] = 1;
                dini_IntSet(file, "AntiSpam", 1);
                format(string, sizeof(string), " Админ %s включил анти-спам", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 3: { ServerInfo[MaxPing] = 1000;
                dini_IntSet(file, "MaxPing", 1000);
                format(string, sizeof(string), " Админ %s включил кик по пингу", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 4: { ServerInfo[ReadCmds] = 1;
                dini_IntSet(file, "ReadCmds", 1);
                format(string, sizeof(string), " Админ %s включил чтение команд", adminname);
                MessageToAdmins(COLOR_GREEN, string); }
            case 5: { ServerInfo[ReadPMs] = 1;
                dini_IntSet(file, "ReadPMs", 1);
                format(string, sizeof(string), " Админ %s включил чтение приватов", adminname);
                MessageToAdmins(COLOR_GREEN, string); }
            case 6: { ServerInfo[NoCaps] = 0;
                dini_IntSet(file, "NoCaps", 0);
                format(string, sizeof(string), " Админ %s включил воможность писать с капс-лок", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 7: { ServerInfo[ConnectMessages] = 1;
                dini_IntSet(file, "ConnectMessages", 1);
                format(string, sizeof(string), " Админ %s включил читать коннект-сообщения", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 8: { ServerInfo[AdminCmdMsg] = 1;
                dini_IntSet(file, "AdminCmdMessages", 1);
                format(string, sizeof(string), " Админ %s включил читать админ-команды", adminname);
                MessageToAdmins(COLOR_GREEN, string); }
            case 9: { ServerInfo[AutoLogin] = 1;
                dini_IntSet(file, "AutoLogin", 1);
                format(string, sizeof(string), " Админ %s включил авто-логин", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
    ...
    if (Current == AdminDisable) {
        ...
        format(file, sizeof(file), "ladmin/config/Config.ini");
        switch (row) {
            case 0 :  { ServerInfo[AntiSwear] = 0;
                dini_IntSet(file, "AntiSwear", 0);
                format(string, sizeof(string), " Админ %s выключил анти-мат", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 1 :  { ServerInfo[NameKick] = 0;
                dini_IntSet(file, "NameKick", 0);
                format(string, sizeof(string), " Админ %s включил кик по нику", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 2: { ServerInfo[AntiSpam] = 0;
                dini_IntSet(file, "AntiSpam", 0);
                format(string, sizeof(string), " Админ %s включил анти-спам", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 3: { ServerInfo[MaxPing] = 0;
                dini_IntSet(file, "MaxPing", 0);
                format(string, sizeof(string), " Админ %s включил кик по пингу", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 4: { ServerInfo[ReadCmds] = 0;
                dini_IntSet(file, "ReadCmds", 0);
                format(string, sizeof(string), " Админ %s включил чтение команд", adminname);
                MessageToAdmins(COLOR_RED, string); }
            case 5: { ServerInfo[ReadPMs] = 0;
                dini_IntSet(file, "ReadPMs", 0);
                format(string, sizeof(string), " Админ %s включил чтение приватов", adminname);
                MessageToAdmins(COLOR_RED, string); }
            case 6: { ServerInfo[NoCaps] = 1;
                dini_IntSet(file, "NoCaps", 1);
                format(string, sizeof(string), " Админ %s включил воможность писать с капс-лок", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 7: { ServerInfo[ConnectMessages] = 0;
                dini_IntSet(file, "ConnectMessages", 0);
                format(string, sizeof(string), " Админ %s включил читать коннект-сообщения", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 8: { ServerInfo[AdminCmdMsg] = 0;
                dini_IntSet(file, "AdminCmdMessages", 0);
                format(string, sizeof(string), " Админ %s включил читать админ-команды", adminname);
                MessageToAdmins(COLOR_RED, string); }
            case 9: { ServerInfo[AutoLogin] = 0;
                dini_IntSet(file, "AutoLogin", 0);
                format(string, sizeof(string), " Админ %s включил авто-логин", adminname);
                SendClientMessageToAll(COLOR_RED, string); }
            case 10 : return ChangeMenu(playerid, Current, LMainMenu);
    ...
    if (Current == LasVenturasMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, 2037.0, 1343.0, 12.0);
                SetPlayerInterior(playerid, 0); } // strip
            case 1 :  { SetPlayerPos(playerid, 2163.0, 1121.0, 23);
                SetPlayerInterior(playerid, 0); } // come a lot
            case 2 :  { SetPlayerPos(playerid, 1688.0, 1615.0, 12.0);
                SetPlayerInterior(playerid, 0); } // lv airport
            case 3 :  { SetPlayerPos(playerid, 2503.0, 2764.0, 10.0);
                SetPlayerInterior(playerid, 0); } // military fuel
            case 4 :  { SetPlayerPos(playerid, 1418.0, 2733.0, 10.0);
                SetPlayerInterior(playerid, 0); } // golf lv
            case 5 :  { SetPlayerPos(playerid, 1377.0, 2196.0, 9.0);
                SetPlayerInterior(playerid, 0); } // pitch match
    ...
    if (Current == LosSantosMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, 2495.0, -1688.0, 13.0);
                SetPlayerInterior(playerid, 0); } // ganton
            case 1 :  { SetPlayerPos(playerid, 1979.0, -2241.0, 13.0);
                SetPlayerInterior(playerid, 0); } // ls airport
            case 2 :  { SetPlayerPos(playerid, 2744.0, -2435.0, 15.0);
                SetPlayerInterior(playerid, 0); } // docks
            case 3 :  { SetPlayerPos(playerid, 1481.0, -1656.0, 14.0);
                SetPlayerInterior(playerid, 0); } // square
            case 4 :  { SetPlayerPos(playerid, 1150.0, -2037.0, 69.0);
                SetPlayerInterior(playerid, 0); } // veradant bluffs
            case 5 :  { SetPlayerPos(playerid, 425.0, -1815.0, 6.0);
                SetPlayerInterior(playerid, 0); } // santa beach
            case 6 :  { SetPlayerPos(playerid, 1240.0, -744.0, 95.0);
                SetPlayerInterior(playerid, 0); } // mullholland
            case 7 :  { SetPlayerPos(playerid, 679.0, -1070.0, 49.0);
                SetPlayerInterior(playerid, 0); } // richman
    ...
    if (Current == SanFierroMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, -1990.0, 137.0, 27.0);
                SetPlayerInterior(playerid, 0); } // sf station
            case 1 :  { SetPlayerPos(playerid, -1528.0, -206.0, 14.0);
                SetPlayerInterior(playerid, 0); } // sf airport
            case 2 :  { SetPlayerPos(playerid, -2709.0, 198.0, 4.0);
                SetPlayerInterior(playerid, 0); } // ocean flats
            case 3 :  { SetPlayerPos(playerid, -2738.0, -295.0, 6.0);
                SetPlayerInterior(playerid, 0); } // avispa country club
            case 4 :  { SetPlayerPos(playerid, -1457.0, 465.0, 7.0);
                SetPlayerInterior(playerid, 0); } // easter basic docks
            case 5 :  { SetPlayerPos(playerid, -1853.0, 1404.0, 7.0);
                SetPlayerInterior(playerid, 0); } // esplanadae north
            case 6 :  { SetPlayerPos(playerid, -2620.0, 1373.0, 7.0);
                SetPlayerInterior(playerid, 0); } // battery point
    ...
    if (Current == DesertMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, 416.0, 2516.0, 16.0);
                SetPlayerInterior(playerid, 0); } // plane graveyard
            case 1 :  { SetPlayerPos(playerid, 81.0, 1920.0, 17.0);
                SetPlayerInterior(playerid, 0); } // area51
            case 2 :  { SetPlayerPos(playerid, -324.0, 1516.0, 75.0);
                SetPlayerInterior(playerid, 0); } // big ear
            case 3 :  { SetPlayerPos(playerid, -640.0, 2051.0, 60.0);
                SetPlayerInterior(playerid, 0); } // dam
            case 4 :  { SetPlayerPos(playerid, -766.0, 1545.0, 27.0);
                SetPlayerInterior(playerid, 0); } // las barrancas
            case 5 :  { SetPlayerPos(playerid, -1514.0, 2597.0, 55.0);
                SetPlayerInterior(playerid, 0); } // el qyebrados
            case 6 :  { SetPlayerPos(playerid, 442.0, 1427.0, 9.0);
                SetPlayerInterior(playerid, 0); } // actane springs
            case 7 : return ChangeMenu(playerid, Current, LTele);
        }
        return TogglePlayerControllable(playerid, true);
    }
    ...
    if (Current == FlintMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, -849.0, -1940.0, 13.0);
                SetPlayerInterior(playerid, 0); } // lake
            case 1 :  { SetPlayerPos(playerid, -1107.0, -1619.0, 76.0);
                SetPlayerInterior(playerid, 0); } // leafy hollow
            case 2 :  { SetPlayerPos(playerid, -1049.0, -1199.0, 128.0);
                SetPlayerInterior(playerid, 0); } // the farm
            case 3 :  { SetPlayerPos(playerid, -1655.0, -2219.0, 32.0);
                SetPlayerInterior(playerid, 0); } // shady cabin
            case 4 :  { SetPlayerPos(playerid, -375.0, -1441.0, 25.0);
                SetPlayerInterior(playerid, 0); } // flint range
            case 5 :  { SetPlayerPos(playerid, -367.0, -1049.0, 59.0);
                SetPlayerInterior(playerid, 0); } // beacon hill
            case 6 :  { SetPlayerPos(playerid, -494.0, -555.0, 25.0);
                SetPlayerInterior(playerid, 0); } // fallen tree
            case 7 : return ChangeMenu(playerid, Current, LTele);
        }
        return TogglePlayerControllable(playerid, true);
    }
    ...
    if (Current == MountChiliadMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, -2308.0, -1657.0, 483.0);
                SetPlayerInterior(playerid, 0); } // chiliad jump
            case 1 :  { SetPlayerPos(playerid, -2331.0, -2180.0, 35.0);
                SetPlayerInterior(playerid, 0); } // bottom chiliad
            case 2 :  { SetPlayerPos(playerid, -2431.0, -1620.0, 526.0);
                SetPlayerInterior(playerid, 0); } // highest point
            case 3 :  { SetPlayerPos(playerid, -2136.0, -1775.0, 208.0);
                SetPlayerInterior(playerid, 0); } // chiliad path
    ...
    if (Current == InteriorsMenu) {
        switch (row) {
            case 0 :  { SetPlayerPos(playerid, 386.5259, 173.6381, 1008.3828);
                SetPlayerInterior(playerid, 3); }
            case 1 :  { SetPlayerPos(playerid, 288.4723, 170.0647, 1007.1794);
                SetPlayerInterior(playerid, 3); }
            case 2 :  { SetPlayerPos(playerid, 372.5565, -131.3607, 1001.4922);
                SetPlayerInterior(playerid, 5); }
            case 3 :  { SetPlayerPos(playerid, -1129.8909, 1057.5424, 1346.4141);
                SetPlayerInterior(playerid, 10); }
            case 4 :  { SetPlayerPos(playerid, 2233.9363, 1711.8038, 1011.6312);
                SetPlayerInterior(playerid, 1); }
            case 5 :  { SetPlayerPos(playerid, 2536.5322, -1294.8425, 1044.125);
                SetPlayerInterior(playerid, 2); }
            case 6 :  { SetPlayerPos(playerid, 1267.8407, -776.9587, 1091.9063);
                SetPlayerInterior(playerid, 5); }
            case 7 :  { SetPlayerPos(playerid, -1421.5618, -663.8262, 1059.5569);
                SetPlayerInterior(playerid, 4); }
            case 8 :  { SetPlayerPos(playerid, -1401.067, 1265.3706, 1039.8672);
                SetPlayerInterior(playerid, 16); }
            case 9 :  { SetPlayerPos(playerid, 285.8361, -39.0166, 1001.5156);
                SetPlayerInterior(playerid, 1); }
            case 10 :  { SetPlayerPos(playerid, 1727.2853, -1642.9451, 20.2254);
    ...
    if (Current == LWeather) {
        ...
        switch (row) {
            case 0: { SetWeather(5);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "SETWEATHER");
                format(string, sizeof(string), " Админ %s сменил погоду", adminname);
                SendClientMessageToAll(blue, string); }
            ...
            case 5: { SetWeather(16);
                ...
                format(string, sizeof(string), " Админ %s сменил погоду", adminname);
                SendClientMessageToAll(blue, string); }
            case 6: { SetWeather(45);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "SETWEATHER");
                format(string, sizeof(string), " Админ %s сменил погоду", adminname);
                SendClientMessageToAll(blue, string); }
            case 7: { SetWeather(44);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "SETWEATHER");
                format(string, sizeof(string), " Админ %s сменил погоду", adminname);
                SendClientMessageToAll(blue, string); }
            case 8: { SetWeather(22);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "SETWEATHER");
                format(string, sizeof(string), " Админ %s сменил погоду", adminname);
                SendClientMessageToAll(blue, string); }
            case 9: { SetWeather(11);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "SETWEATHER");
                format(string, sizeof(string), " Админ %s сменил погоду", adminname);
                SendClientMessageToAll(blue, string); }
    ...
    if (Current == LTuneMenu) {
        switch (row) {
            case 0: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1010);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 1: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1087);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 2: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1081);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 3 :  { AddVehicleComponent(GetPlayerVehicleID(playerid), 1078);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 4: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1098);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 5: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1074);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 6: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1082);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 7: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1085);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 8: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1025);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
            case 9: { AddVehicleComponent(GetPlayerVehicleID(playerid), 1077);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Компонент в машине создан"); }
    ...
    if (Current == PaintMenu) {
        switch (row) {
            case 0: { ChangeVehiclePaintjob(GetPlayerVehicleID(playerid), 0);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена винилами № 1"); }
            case 1: { ChangeVehiclePaintjob(GetPlayerVehicleID(playerid), 1);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена винилами № 2"); }
            case 2: { ChangeVehiclePaintjob(GetPlayerVehicleID(playerid), 2);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена винилами № 3"); }
            case 3: { ChangeVehiclePaintjob(GetPlayerVehicleID(playerid), 3);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена винилами № 4"); }
            case 4: { ChangeVehiclePaintjob(GetPlayerVehicleID(playerid), 4);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена винилами № 5"); }
            case 5: { ChangeVehicleColor(GetPlayerVehicleID(playerid), 0, 0);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена в черный цвет"); }
            case 6: { ChangeVehicleColor(GetPlayerVehicleID(playerid), 1, 1);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена в белый цвет"); }
            case 7: { ChangeVehicleColor(GetPlayerVehicleID(playerid), 79, 158);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена черными винилами"); }
            case 8: { ChangeVehicleColor(GetPlayerVehicleID(playerid), 146, 183);
                PlayerPlaySound(playerid, 1133, 0.0, 0.0, 0.0);
                SendClientMessage(playerid, COLOR_GREEN, " Машина покрашена черными винилами"); }
    ...
    if (Current == LTime) {
        ...
        switch (row) {
            case 0: { for (new i = 0; i < MAX_PLAYERS; i++)
                    if (IsPlayerConnected(i)) SetPlayerTime(i, 7, 0);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "LTIME MENU");
                format(string, sizeof(string), " Админ %s сменил время", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 1: { for (new i = 0; i < MAX_PLAYERS; i++)
                ...
                format(string, sizeof(string), " Админ %s сменил время", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 2: { for (new i = 0; i < MAX_PLAYERS; i++)
                    if (IsPlayerConnected(i)) SetPlayerTime(i, 16, 0);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "LTIME MENU");
                format(string, sizeof(string), " Админ %s сменил время", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 3: { for (new i = 0; i < MAX_PLAYERS; i++)
                    if (IsPlayerConnected(i)) SetPlayerTime(i, 20, 0);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "LTIME MENU");
                format(string, sizeof(string), " Админ %s сменил время", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
            case 4: { for (new i = 0; i < MAX_PLAYERS; i++)
                    if (IsPlayerConnected(i)) SetPlayerTime(i, 0, 0);
                PlayerPlaySound(playerid, 1057, 0.0, 0.0, 0.0);
                CMDMessageToAdmins(playerid, "LTIME MENU");
                format(string, sizeof(string), " Админ %s сменил время", adminname);
                SendClientMessageToAll(COLOR_GREEN, string); }
...
public Duel(player1, player2) {
    if (cdt[player1] == 6) {
        GameTextForPlayer(player1, "~p~DYELJ HA4ALACJ...", 1000, 6);
        GameTextForPlayer(player2, "~p~DYELJ HA4ALACJ...", 1000, 6);
    }
    ...
    if (cdt[player1] == 0) {
        ...
        GameTextForPlayer(player1, "~g~FO~ r~!", 1000, 6);
        GameTextForPlayer(player2, "~g~FO~ r~!", 1000, 6);
        return 0;
    } else {
        new text[7];
        format(text, sizeof(text), "~w~%d", cdt[player1]);
        ...
        GameTextForPlayer(player1, text, 1000, 6);
        GameTextForPlayer(player2, text, 1000, 6);
    }
...
public UnFreezeMe(player1) {
    ...
    PlayerPlaySound(player1, 1057, 0.0, 0.0, 0.0);
    GameTextForPlayer(player1, "~g~PA3MOPOGEH", 3000, 3);
}
...
public PingKick() {
    ...
    for (new i = 0; i < MAX_PLAYERS; i++) {
        if (IsPlayerConnected(i) && PlayerInfo[i][Level] == 0) {
            ...
            if (ammo > 1 && weap == 38) {
                new string[128];
                format(string, sizeof(string), " ИНФО: у игрока %s содержится миниган с %d патронами", PlayerName2(i), ammo);
...
public ConnectedPlayers() {
    new Connected;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i)) Connected++;
    return Connected;
}
...
public JailedPlayers() {
    new JailedCount;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i) && PlayerInfo[i][Jailed] == 1) JailedCount++;
    return JailedCount;
}
...
public FrozenPlayers() {
    new FrozenCount;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i) && PlayerInfo[i][Frozen] == 1) FrozenCount++;
    return FrozenCount;
}
...
public MutedPlayers() {
    new Count;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i) && PlayerInfo[i][Muted] == 1) Count++;
    return Count;
}
...
public InVehCount() {
    new InVeh;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i) && IsPlayerInAnyVehicle(i)) InVeh++;
    return InVeh;
}
...
public AdminCount() {
    new LAdminCount;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i) && PlayerInfo[i][Level] >= 1) LAdminCount++;
    return LAdminCount;
}
...
public RconAdminCount() {
    new rAdminCount;
    for (new i = 0; i < MAX_PLAYERS; i++)
        if (IsPlayerConnected(i) && IsPlayerAdmin(i)) rAdminCount++;
    return rAdminCount;
}
...
public MessageToAdmins(color, const string[]) {
    for (new i = 0; i < MAX_PLAYERS; i++) {
        if (IsPlayerConnected(i) == 1)
            if (PlayerInfo[i][Level] >= 1) SendClientMessage(i, color, string);
    }
    return 1;
}
...
SavePlayer(playerid) {
    ...
    dUserSetINT(PlayerName2(playerid)).("weap1", weap1);
    dUserSetINT(PlayerName2(playerid)).("weap1ammo", ammo1);
    dUserSetINT(PlayerName2(playerid)).("weap2", weap2);
    dUserSetINT(PlayerName2(playerid)).("weap2ammo", ammo2);
    dUserSetINT(PlayerName2(playerid)).("weap3", weap3);
    dUserSetINT(PlayerName2(playerid)).("weap3ammo", ammo3);
    dUserSetINT(PlayerName2(playerid)).("weap4", weap4);
    dUserSetINT(PlayerName2(playerid)).("weap4ammo", ammo4);
    dUserSetINT(PlayerName2(playerid)).("weap5", weap5);
    dUserSetINT(PlayerName2(playerid)).("weap5ammo", ammo5);
    dUserSetINT(PlayerName2(playerid)).("weap6", weap6);
    dUserSetINT(PlayerName2(playerid)).("weap6ammo", ammo6);
...
#if defined USE_MENUS
DestroyAllMenus() {
    DestroyMenu(LVehicles);
    DestroyMenu(twodoor);
    DestroyMenu(fourdoor);
    DestroyMenu(fastcar);
    DestroyMenu(Othercars);
    DestroyMenu(bikes);
    DestroyMenu(boats);
    DestroyMenu(planes);
    DestroyMenu(helicopters);
    DestroyMenu(LTime);
    DestroyMenu(XWeapons);
    DestroyMenu(XWeaponsBig);
    DestroyMenu(XWeaponsSmall);
    DestroyMenu(XWeaponsMore);
    DestroyMenu(LWeather);
    DestroyMenu(LTuneMenu);
    DestroyMenu(PaintMenu);
    DestroyMenu(LCars);
    DestroyMenu(LCars2);
    DestroyMenu(LTele);
...
stock ConfigInConsole() {
    ...
    if (ServerInfo[AntiSwear] == 0) print("  Anti Swear:              Disabled ");
    else print("  Anti Swear:             Enabled ");
    if (ServerInfo[AntiSpam] == 0) print("  Anti Spam:               Disabled ");
    else print("  Anti Spam:              Enabled ");
    if (ServerInfo[ReadCmds] == 0) print("  Read Cmds:               Disabled ");
    else print("  Read Cmds:              Enabled ");
    if (ServerInfo[ReadPMs] == 0) print("  Read PMs:                Disabled ");
    else print("  Read PMs:               Enabled ");
    if (ServerInfo[ConnectMessages] == 0) print("  Connect Messages:        Disabled ");
    else print("  Connect Messages:       Enabled ");
    if (ServerInfo[AdminCmdMsg] == 0) print("  Admin Cmd Messages:     Disabled ");
    else print("  Admin Cmd Messages:     Enabled ");
    ...
    if (ServerInfo[AdminOnlySkins] == 0) print("  AdminOnlySkins:         Disabled ");
    else print("  AdminOnlySkins:         Enabled ");
    ...
    if (ServerInfo[AntiBot] == 0) print("  Anti Bot:                Disabled ");
    else print("  Anti Bot:                Enabled ");
    ...
    if (ServerInfo[MaxPing] == 0) print("  Ping Control:            Disabled");
    else print("  Ping Control:            Enabled");
    printf("  Max Ping:                %d\n", ServerInfo[MaxPing]);
    print(" __________ Players ______________");
    if (ServerInfo[GiveWeap] == 0) print("  Save/Give Weaps:         Disabled");
    else print("  Save/Give Weaps:         Enabled");
    ...
    if (ServerInfo[Locked] == 0) print("  Server Locked:           No");
    else print("  Server Locked:           Yes");
...
//=====================[ Configuration ] =======================================
stock UpdateConfig() {
    new file[256], File:file2, string[100];
    format(file, sizeof(file), "ladmin/config/Config.ini");
    ForbiddenWordCount = 0;
    BadNameCount = 0;
    BadPartNameCount = 0;
    ...
    if ((file2 = fopen("ladmin/config/ForbiddenNames.cfg", io_read))) {
        ...
        fclose(file2);
        printf(" -%d Forbidden Names Loaded", BadNameCount);
    }
    ...
    if ((file2 = fopen("ladmin/config/ForbiddenPartNames.cfg", io_read))) {
        ...
        fclose(file2);
        printf(" -%d Forbidden Tags Loaded", BadPartNameCount);
    }
    ...
    if ((file2 = fopen("ladmin/config/ForbiddenWords.cfg", io_read))) {
        ...
        fclose(file2);
        printf(" -%d Forbidden Words Loaded", ForbiddenWordCount);
    }
}
//=====================[ SAVING DATA ] =========================================
